class SellerProfile {
  ...
  factory SellerProfile.fromSupabase(
    ...
    final ownerName = user?['full_name'] as String? ??
        (row['full_name'] as String? ?? (username.isNotEmpty ? username : 'Seller'));
    ...
    final rating = (user?['rating_average'] as num?)?.toDouble() ??
        (row['rating_average'] as num?)?.toDouble() ??
        (row['rating'] as num?)?.toDouble() ??
        5.0;

    final ratingCount = (user?['rating_count'] as int?) ??
    ...
    final sales = (row['total_sales'] as int?) ?? 0;
    final followerCount = (row['follower_count'] as int?) ?? 0;
    final followingCount = (row['following_count'] as int?) ?? 0;
    final trustScore = (user?['trust_score'] as num?)?.toInt() ?? 80;
    final shopBio = row['shop_bio'] as String? ??
        user?['bio'] as String? ??
        row['bio'] as String?;
    final city = row['city'] as String? ??
        ...
        '';
    final isApproved = (row['is_approved'] as bool?) ??
        (user?['role'] == 'seller');
